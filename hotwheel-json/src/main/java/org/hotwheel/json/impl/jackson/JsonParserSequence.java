 ...
 * {@link JacksonParser}s to create a single logical sequence of
 * tokens, as a single {@link JacksonParser}.
 * <p>
...
public class JsonParserSequence extends JsonParserDelegate {
    ...
    protected JsonParserSequence(JacksonParser[] parsers) {
        super(parsers[0]);
        _parsers = parsers;
        _nextParser = 1;
    }
     ...
     */
    public static JsonParserSequence createFlattened(JacksonParser first, JacksonParser second) {
        if (!(first instanceof JsonParserSequence || second instanceof JsonParserSequence)) {
            // simple:
            return new JsonParserSequence(new JacksonParser[]{first, second});
        }
        ArrayList<JacksonParser> p = new ArrayList<JacksonParser>();
        ...
        return new JsonParserSequence(p.toArray(new JacksonParser[p.size()]));
    }

    protected void addFlattenedActiveParsers(List<JacksonParser> result) {
        for (int i = _nextParser - 1, len = _parsers.length; i < len; ++i) {
            JacksonParser p = _parsers[i];
