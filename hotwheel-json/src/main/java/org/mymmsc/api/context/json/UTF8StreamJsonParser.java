...
/**
 * This is a concrete implementation of {@link JacksonParser}, which is
...
public final class UTF8StreamJsonParser
    ...
    protected void _closeInput() throws IOException {
         ...
         */
        if (_inputStream != null) {
            if (_ioContext.isResourceManaged() || isEnabled(JacksonParser.Feature.AUTO_CLOSE_SOURCE)) {
                _inputStream.close();
            }
            _inputStream = null;
        }
    ...
    private final int _verifyNoLeadingZeroes()
        ...
        if (!isEnabled(JacksonParser.Feature.ALLOW_NUMERIC_LEADING_ZEROS)) {
    ...
    protected final Name _handleUnusualFieldName(int ch)
        ...
        if (ch == INT_APOSTROPHE && isEnabled(JacksonParser.Feature.ALLOW_SINGLE_QUOTES)) {
            return _parseApostropheFieldName();
        }
        // [JACKSON-69]: allow unquoted names if feature enabled:
        if (!isEnabled(JacksonParser.Feature.ALLOW_UNQUOTED_FIELD_NAMES)) {
    ...
    protected JsonToken _handleUnexpectedValue(int c)
        ...
        switch (c) {
            ...
            case 'N':
                _matchToken("NaN", 1);
                if (isEnabled(JacksonParser.Feature.ALLOW_NON_NUMERIC_NUMBERS)) {
    ...
    private final void _skipComment()
            throws IOException, JsonParseException {
        if (!isEnabled(JacksonParser.Feature.ALLOW_COMMENTS)) {
