 ...
 */
package org.mymmsc.sql.jdbc.pool;

import org.mymmsc.sql.jdbc.pool.jmx.ConnectionPoolMBean;
 ...
 * As the underlying implementation, the connection pool uses {@link BlockingQueue} to store active and idle connections.
...
public class ConnectionPool {
     ...
     */
    public static final String POOL_JMX_DOMAIN = "mymmsc.jdbc";
    /**
     ...
     */
    protected org.mymmsc.sql.jdbc.pool.jmx.ConnectionPool jmxPool = null;
     ...
     * This Proxy implements {@link Connection} and {@link javax.sql.PooledConnection} interfaces.
     * All calls on {@link Connection} methods will be propagated down to the actual JDBC connection except for the
     * {@link Connection#close()} method.
     * @param con a {@link PooledConnection} to wrap in a Proxy
     * @return a {@link Connection} object wrapping a pooled connection.
    ...
    /**
     * Creates and caches a {@link Constructor} used to instantiate the proxy object.
    ...
    public Constructor<?> getProxyConstructor(boolean xa) throws NoSuchMethodException {
        ...
        if (proxyClassConstructor == null ) {
            Class<?> proxyClass = xa ?
                Proxy.getProxyClass(ConnectionPool.class.getClassLoader(), new Class[] {Connection.class,javax.sql.PooledConnection.class, javax.sql.XAConnection.class}) :
                Proxy.getProxyClass(ConnectionPool.class.getClassLoader(), new Class[] {Connection.class,javax.sql.PooledConnection.class});
     ...
     * Active connections will be closed upon the {@link Connection#close close} method is called
    ...
    protected void init(PoolConfiguration properties) throws SQLException {
        ...
        for (int i=0; i<proxies.length; i++) {
            try {
                ...
                if (jmxPool!=null) jmxPool.notify(org.mymmsc.sql.jdbc.pool.jmx.ConnectionPool.NOTIFY_INIT, getStackTrace(x));
                close(true);
...
//===============================================================================
    ...
    protected void abandon(PooledConnection con) {
        ...
        try {
            ...
            if (jmxPool!=null) {
                jmxPool.notify(org.mymmsc.sql.jdbc.pool.jmx.ConnectionPool.NOTIFY_ABANDON, trace);
            }
    ...
    protected void suspect(PooledConnection con) {
        ...
        try {
            ...
            if (jmxPool!=null) {
                jmxPool.notify(org.mymmsc.sql.jdbc.pool.jmx.ConnectionPool.SUSPECT_ABANDONED_NOTIFICATION, trace);
            }
            con.setSuspect(true);
        } finally {
            con.unlock();
    ...
    private PooledConnection borrowConnection(int wait, String username, String password) throws SQLException {
        ...
        while (true) {
            ...
            if (maxWait==0 && con == null) { //no wait, return one if we have one
                if (jmxPool!=null) {
                    jmxPool.notify(org.mymmsc.sql.jdbc.pool.jmx.ConnectionPool.POOL_EMPTY, "Pool empty - no wait.");
                }
            ...
            if (con == null) {
                if ((System.currentTimeMillis() - now) >= maxWait) {
                    if (jmxPool!=null) {
                        jmxPool.notify(org.mymmsc.sql.jdbc.pool.jmx.ConnectionPool.POOL_EMPTY, "Pool empty - timeout.");
                    }
                    throw new PoolExhaustedException("[" + Thread.currentThread().getName()+"] " +
        ...
        if (con != null) {
            try {
                ...
                if (con.isSuspect()) {
                    ...
                    if (jmxPool!=null) {
                        jmxPool.notify(org.mymmsc.sql.jdbc.pool.jmx.ConnectionPool.SUSPECT_RETURNED_NOTIFICATION,
     ...
     * TODO investigate if we simply should store {@link Thread#getStackTrace()} elements
     */
     ...
     * @return the object implementing the {@link ConnectionPoolMBean} interface
     */
    public org.mymmsc.sql.jdbc.pool.jmx.ConnectionPool getJmxPool() {
        return jmxPool;
    }
    ...
    protected void createMBean() {
        try {
            jmxPool = new org.mymmsc.sql.jdbc.pool.jmx.ConnectionPool(this);
        } catch (Exception x) {
    ...
    private static class PrivilegedNewTimer implements PrivilegedAction<Timer> {
        @Override
        public Timer run() {
            return new Timer("MyMMSC JDBC Pool Cleaner["+ System.identityHashCode(ConnectionPool.class.getClassLoader()) + ":"+
