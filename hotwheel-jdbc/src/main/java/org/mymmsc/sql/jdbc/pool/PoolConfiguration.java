...
import org.mymmsc.sql.jdbc.pool.interceptor.ConnectionState;
import org.mymmsc.sql.jdbc.pool.interceptor.ResetAbandonedTimer;
import org.mymmsc.sql.jdbc.pool.interceptor.StatementFinalizer;
...
public interface PoolConfiguration {
     ...
     */
    public static final String PKG_PREFIX = "org.mymmsc.sql.jdbc.pool.interceptor.";
    ...
    /**
     * The fully qualified Java class name of the JDBC driver to be used. The driver has to be accessible from the same classloader as mymmsc-jdbc.jar
    ...
    /**
     * A semicolon separated list of classnames extending {@link JdbcInterceptor} class.
     * These interceptors will be inserted as an interceptor into the chain of operations on a java.sql.Connection object.
     * Example interceptors are {@link StatementFinalizer StatementFinalizer} to close all
     * used statements during the session.
     * {@link ResetAbandonedTimer ResetAbandonedTimer} resets the timer upon every operation
     * on the connection or a statement.
     * {@link ConnectionState ConnectionState} caches the auto commit, read only and catalog settings to avoid round trips to the DB.
     * The default value is null.
    ...
    /**
     * If set to true, the connection pool creates a {@link ConnectionPoolMBean} object
