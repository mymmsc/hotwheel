...
public class PoolProperties implements PoolConfiguration, Cloneable, Serializable {
    ...
    private static final Logger log = LoggerFactory.getLogger(PoolProperties.class);
    ...
    private volatile String name = "MyMMSC Connection Pool["+(poolCounter.addAndGet(1))+"-"+System.identityHashCode(PoolProperties.class)+"]";
        ...
        public Class<? extends JdbcInterceptor> getInterceptorClass() throws ClassNotFoundException {
            if (clazz==null) {
                if (getClassName().indexOf('.')<0) {
                    ...
                    clazz = Class.forName(PKG_PREFIX +getClassName(), true, this.getClass().getClassLoader());
                } else {
                    if (log.isDebugEnabled()) {
