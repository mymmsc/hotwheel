 ...
 */
package org.mymmsc.sql.impl;

import org.mymmsc.api.assembly.Api;
import org.mymmsc.api.assembly.RegExp;
import org.mymmsc.sql.AbstractScript;
import org.mymmsc.sql.ScriptParser;
import org.mymmsc.sql.app.FieldObject;
            ...
            try {
                ...
                while ((sbuf = reader.readLine()) != null) {
                    ...
                    if (sbuf.indexOf("CREATE TABLE") >= 0) {
                        ...
                        // 字段解析 结束
                        logger.info("增加数据表: " + sTable);
                        bField = false;
                        to.setFields(listFields);
                        // listTables.add(to);
                        put(sTable, to);
                        logger.info("目前完成数据表结构分析: " + getTableNumber());
                        to = null;
